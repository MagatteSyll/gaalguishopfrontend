...
function DesactivationSansAvertissement(props){
  ...
  setmotif(e.target.value)
   ...
   const notify = () => toast.success("boutique désactivée avec succes!",{
 position:toast.POSITION.TOP_CENTER,
  ...
  // axios
   // .post('http://127.0.0.1:8000/api/staff/desactivationboutique/',{id:id,motif:motif},
   // {headers:{
   //  'Authorization': `JWT ${localStorage.getItem('__jdkm__')}` 
   // }})
  axiosInstance
  .post('staff/desactivationboutique/',{id:id,motif:motif})
   .then(res=>{
...
return(
  ...
  {load && nom===boutique.user.prenom+""+boutique.user.nom?
   ...
   > 
    <p>Precisez le motif de la désactivation </p>
     <textarea
     required 
...
export default DesactivationSansAvertissement